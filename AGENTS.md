...
```
openrouter:anthropic/Codex-sonnet-4  → Cloud via OpenRouter
...
anthropic:Codex-sonnet-4             → Direct Anthropic connection
...
9. **Custom Endpoint Icons**: Models from custom endpoints may match name patterns (e.g., "Codex"). Check `custom:` prefix first.
